...
let firstCivilizationToSentMessage = [];
  lines.some((value) => {
  if (!parseInt(value) && parseInt(value) !== 0) {
    const [planetName, yearArrivedMessage, yearsAgo] = value.split(" ");
    const yearArrived = parseInt(yearArrivedMessage);
    const yearsAgoToSentMessage = parseInt(yearsAgo);
    const yearSentMessage = yearArrived - yearsAgoToSentMessage;
    firstCivilizationToSentMessage.push({
      planetName,
      yearSentMessage,
    });
  }
   else if (firstCivilizationToSentMessage.length) {
    firstCivilizationToSentMessage.sort(
      (a, b) => a.yearSentMessage - b.yearSentMessage,
    );
   console.log(firstCivilizationToSentMessage[0].planetName);

    firstCivilizationToSentMessage = [];
  }
